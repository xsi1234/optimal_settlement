...
function maximize_subroutine(y0,cut_indices0,net_edges0,vert_indices,vert_neighs,arcs,x,mass,lambda1,alpha,tol,rho0,...
    ...
    for j = 1:outer_iter
        ...
        p = 3;
        h = 0.005;
        Adj2 = zeros(size(y2,1));
        for i = 1:size(net_edges2, 1)
            Adj2(net_edges2(i,1), net_edges2(i,2)) = 1;
        end
        Adj2 = or(Adj2, Adj2');
        figure();
        hold on;
        plot_network(y2, Adj2); 
        plot_particles(x, color_mat);
        x = euler_wrap(x, y2, Adj2, mass, inner_iter, p, h, alpha_2, theta, sigma, lambda1, color_mat);
    end
end
