function [y_new,z_new,b_new,net_edges,y_net_edge_inds] = modifySpacingAllComps1(y,r,top_struct,num_add,eo,p,FIX_ENDPTS)
...
singletons = find(cellfun(@length,neighbors(end_pts_singletons))<1);
...
%     end
% end
proj_indices = find(edges(:,1)>m & edges(:,2)<=m);
for i=1:m
...
split_edges_inds = find(ismember(net_edges(:,1),vert_indices) & ismember(net_edges(:,2),vert_indices));
...
k = max(k2 - num_add,0); %number of removals 
...
%     end
    net_edges = [net_edges;[m+1,neigh];[m+1,split(i)]]; % add edges from new point to split(i) and neigh
