...
%computeProjs computes the projections to the polygonal curve, given the
%closest point in y.
%   this function only computes estimated projections onto edges of y,
%   given projections onto the vertices of y. More precisely, according to
%   this function only data which project to either y_i or y_{i+1}
%   project to the edge (y_i, y_{i+1}), which is not necessarily true.
...
%singletons = intersect(start_indices,end_indices);
singleton_ind = start_indices==end_indices;
singletons = start_indices(singleton_ind);
%start_indices = setdiff(start_indices, singletons);
start_indices = start_indices(~singleton_ind);
%end_indices = setdiff(end_indices, singletons);
...
if ~isempty(x_aff1i)
    ...
    %x_aff = x_aff1(find(dot((y(I(x_aff1),:)-x(x_aff1,:))',v(I(x_aff1),:)') < 0));
...
if ~isempty(x_aff2i)
    ...
    %x_aff = x_aff2(find(dot((y(I(x_aff2),:)-x(x_aff2,:))',v(I(x_aff2)-1,:)') > 0));
...
%x_aff = setdiff(setdiff(1:n,union(x_aff1,x_aff2)),find(ismember(I,singletons)));
...
if ~isempty(x_affi)
    ...
    %x_aff_r = x_aff(find(right_dot > 0 & right_dot>left_dot));
    ...
    %x_aff_l = x_aff(find(left_dot > 0 & left_dot>right_dot));
